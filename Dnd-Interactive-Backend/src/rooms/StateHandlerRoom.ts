...
import { State, IState, LoadSaveHistory, LoadImage, LoadCampaign, GameStateEnum } from "dnd-interactive-shared";
...
import { PlayerMovementHistoryDAO, PlayerMovementHistoryDB } from "../Database/Tables/PlayerMovementHistoryDB";
import { EnemyMovementHistoryDAO, EnemyMovementHistoryDB } from "../Database/Tables/EnemyMovementHistoryDB";
...
export class StateHandlerRoom extends Room<State> {
    ...
    onCreate(options: IState) {
        ...
        this.onMessage("changePlayerColor", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "color", type: "string", PostProcess: undefined }];
        ...
        this.onMessage("changePlayerTotalHp", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "totalHp", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("changePlayerHp", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "hp", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("playerDeathAdd", (client, data) => {
            if (!this.softAuthenticate(client.sessionId, data.clientToChange)) return;
            try {
                const inputList: ValidationInputType[] = [{ name: "clientToChange", type: "string", PostProcess: undefined }];
        ...
        this.onMessage("setGameState", (client, data) => {
            if (!this.authenticateHostAction(client.sessionId)) return;
            try {
                const inputList: ValidationInputType[] = [{ name: "gameState", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("setPlayerSize", (client, data) => {
            if (!this.authenticateHostAction(client.sessionId)) return;
            try {
                const inputList: ValidationInputType[] = [{ name: "size", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("addDrawings", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "points", type: "array", PostProcess: undefined }];
        ...
        this.onMessage("removeDrawing", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "playerId", type: "string", PostProcess: undefined }];
        ...
        this.onMessage("removeFog", (client, data) => {
            if (!this.authenticateHostAction(client.sessionId)) return;
            try {
                const inputList: ValidationInputType[] = [{ name: "id", type: "string", PostProcess: undefined }];
            ...
            try {
                ...
                ImageCatalogDB.getInstance()
                    ...
                    .then((value) => {
                        if (value === undefined) return;
                        MapDB.getInstance()
                            .create(new MapDAO(value.img_catalog_id!, validateParams.iconHeight, validateParams.name, player.userId))
                            .then((index) => {
                                const mData: any = {
                                    id: index,
                                ...
                                SaveHistoryDB.getInstance().create(new SaveHistoryDAO(new Date(), mData.id, player.userId));
            ...
            SaveHistoryDB.getInstance()
                ...
                .then((index) => {
                    ...
                    [...data.players.keys()].forEach((key) => {
                        ...
                        PlayerMovementHistoryDB.getInstance().create(new PlayerMovementHistoryDAO(index, key, position, initiative, health, totalHealth, deathSaves, lifeSaves));
                    });
                    ...
                    [...data.map.enemy.keys()].forEach((key) => {
                        ...
                        EnemyMovementHistoryDB.getInstance().create(new EnemyMovementHistoryDAO(index, +key, size, position, initiaitive, health, totalHealth, deathSaves, lifeSaves));
                    });
                    ...
                    InitiativeHistoryDB.getInstance().create(new InitiativeHistoryDAO(index, data.map.initiativeIndex));
                });
...
delete from Public."Map" where player_id = 'temp';
    */
            ...
            MapDB.getInstance()
                ...
                .then((value: LoadCampaign[] | undefined) => {
                    const sendData: LoadCampaign[] | undefined = value?.map((val) => {
                        return { id: val.id, image_name: val.image_name, name: val.name, height: val.height, width: val.width };
                    });
        ...
        this.onMessage("getVersionsByCampaign", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "campaign_id", type: "string", PostProcess: undefined }];
        ...
        this.onMessage("loadMap", async (client, data) => {
            ...
            try {
                ...
                const playerHistoryPromise = PlayerMovementHistoryDB.getInstance().getPlayerMovementAtHistoryId(validateParams.history_id);
                const enemyHistoryPromise = EnemyMovementHistoryDB.getInstance().getEnemyMovementAtHistoryId(validateParams.history_id);
                const fogHistoryPromise = FogStateHistoryDB.getInstance().getFogStateAtHistoryId(validateParams.history_id);
                const ininitiativePromise = InitiativeHistoryDB.getInstance().selectByHistoryId(validateParams.history_id);

                const savedHistoryData = await Promise.all([mapPromise, playerHistoryPromise, enemyHistoryPromise, fogHistoryPromise, ininitiativePromise]);
        ...
        this.onMessage("ChangeAudio", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "index", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("SetTimestamp", (client, data) => {
            if (!this.authenticateHostAction(client.sessionId)) return;
            try {
                const inputList: ValidationInputType[] = [{ name: "timestamp", type: "number", PostProcess: undefined }];
        ...
        this.onMessage("AddAudio", (client, data) => {
            try {
                const inputList: ValidationInputType[] = [{ name: "audioName", type: "string", PostProcess: undefined }];
   ...
   */
    ...
    authenticateHostAction(session_id: string) {
        ...
        return this.state.currentHostUserId !== undefined && user.isHost && this.state.currentHostUserId === user.userId;
    }
