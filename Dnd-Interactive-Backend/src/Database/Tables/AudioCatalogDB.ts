...
export class AudioCatalogDAO extends DAO {
  ...
  constructor(player_id: string, audio_name: string, img_catalog_id?: number) {
    super();
...
export class AudioCatalogDB extends DatabaseBase<AudioCatalogDAO> {
  private static instance: AudioCatalogDB | undefined = undefined;
  public static getInstance(): AudioCatalogDB {
    if (AudioCatalogDB.instance === undefined) AudioCatalogDB.instance = new AudioCatalogDB();
    return AudioCatalogDB.instance;
  }

  async selectAllAudioByPlayerId(player_id: string) {
    const query = `SELECT * FROM public."Audio_Catalog" where player_id = $1;`;

    const result: QueryResult<AudioCatalogDAO> | undefined = await Database.getInstance()
  ...
  async create(data: AudioCatalogDAO): Promise<number | undefined> {
