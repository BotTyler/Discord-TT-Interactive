...
export default function PlayerController({ player }: { player: Player }) {
  ...
  return isConnected && !isHost ? <PlayerMarker playerId={player.userId} playerAvatar={player.avatarUri} size={iconSize} position={L.latLng(position.lat, position.lng)} color={color} key={`MovementMarkerController-${player.userId}`} /> : "";
}
